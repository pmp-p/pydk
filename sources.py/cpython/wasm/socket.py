...
# implemented in Python FOR EMSCRIPTEN CPYTHON.
...
class DomSocket_client:
    ...
    def setblocking(self, *argv, **kw):
        if self._fileno:
            print("149:setblocking", self, argv, kw)
            fcntl.fcntl(self._fileno, fcntl.F_SETFL, os.O_NONBLOCK)
        else:
            print("152:ERROR:setblocking", self, argv, kw)

    def setsockopt(self, *argv, **kw):
        print("155:setsockopt", self, argv, kw)

    def pump(self, limit=0):
        read = []
        while True:
            (ready, junk, junk) = select.select([self._fileno], [], [], 0.000_001)
            if not len(ready):
                break
            read.append(self.rfile.read())
            if limit and limit == len(ready):
                break

        return b"".join(read)

    def recv(self, sz=0):
        print(f"{self} READ {sz}")
        self.data.append(self.pump(sz))
        try:
    ...
    def send(self, data):
        print(f"{self} SEND %r" % data)
        self.wfile.write(b("echo: %s" % data))
        self.wfile.flush()
        return len(data)
...
class DomSocket_server(DomSocket_client):
    ...
    def bind(self, *argv, **kw):
        print("186:bind", argv, kw)

        (self.addr, self.port) = argv[0]
    ...
    def accept(self):
        self.data.append(self.pump())
        # not ioctl ?
        if not self.data[-1]:
            print("217:accept-reject", self)
            print()
...
def socket(family=-1, type=-1, proto=-1, fileno=None):
    ...
    return native_socket(family, type, proto, fileno)


class native_socket(_socket.socket):
    ...
    def __init__(self, family=-1, type=-1, proto=-1, fileno=None):
        ...
        print("151:", family, type, proto, fileno)
    ...
    if __EMSCRIPTEN__:
        ...
        def setblocking(self, block):
            if block:
                raise Exception("Blocking not supported on browser")
