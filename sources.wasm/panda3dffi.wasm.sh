...
if true
then
OPT_COMMON="--optimize 3 --use-freetype --use-bullet --use-zlib --no-openssl --no-eigen --use-direct"
...
EM_LIBS="-s USE_LIBPNG=1 -s USE_FREETYPE=1 -s USE_OGG=1 -s USE_BULLET=1 -s USE_ZLIB=1"
...
then
    ...
    EM_LIBS="$EM_LIBS -s USE_VORBIS=1 -s USE_HARFBUZZ=1 "
else
    EM_LIBS="$EM_LIBS -s USE_HARFBUZZ=1 "
    #TP_HB="--no-harfbuzz"  # pic problem?
    TP_VB="--no-vorbis"
fi
