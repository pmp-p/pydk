...
ExternalProject_Add(
    ...
    CMAKE_ARGS -DCMAKE_INSTALL_PREFIX=${HOST} \
-DCMAKE_CURRENT_BINARY_DIR=${HOST}/bin -DCMAKE_INSTALL_BINDIR=${HOST}/bin -DCMAKE_INSTALL_INCLUDEDIR=${HOST}/include -DCMAKE_INSTALL_LIBDIR=${HOST}/lib \
-DCMAKE_INSTALL_BINDIR=${HOST}/bin \
-DPython_EXECUTABLE=${HOST}/bin/python${PYMAJOR}.${PYMINOR} -DPYTHON_EXECUTABLE=${HOST}/bin/python${PYMAJOR}.${PYMINOR}\
 ${PANDA3D_CMAKE_ARGS}
...
END
        ...
        PANDA3D_ACMAKE="$CMAKE ${BUILD_SRC}/${unit}-prefix/src/${unit} -Wno-dev\
     ...
     -DCMAKE_INSTALL_INCLUDEDIR=${APKUSR}/include -DCMAKE_INSTALL_BINDIR=${APKUSR}/lib -DCMAKE_INSTALL_BINDIR=${APKUSR}/bin \
     ${PANDA3D_CMAKE_ARGS_COMMON}"
